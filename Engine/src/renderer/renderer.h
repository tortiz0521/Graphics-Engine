...
namespace {
    ...
    {
        int key = e->keyCode & 0xFF;

        if (key < 512)
        {
            keys[key] = (eventType == EMSCRIPTEN_EVENT_KEYDOWN);
        }
        ...
        if (eventType == EMSCRIPTEN_EVENT_KEYDOWN)
        {
            ...
            {
                // No window close in browser, you typically signal engine shutdown
                closeWindow = true;
            }
        }
        return EM_TRUE;
    ...
    {
        float xpos = (float)e->targetX;
        float ypos = (float)e->targetY;
...
public:
    ...
    void BindUBOs(const std::vector<uint32_t>& IDs);
